...
for i=1:Ny
    ...
    for j1=1:Nx
        if j1 == 1
            a1x(1,j1)=2+a*86400*(-U(i,j1+2)+8*U(i,j1+1)-8*U(i,Nx)+U(i,Nx-1)-Uo(i,j1+2)+8*Uo(i,j1+1)-8*Uo(i,Nx)+Uo(i,Nx-1))+30*(a/dx)*K(i,j1);%-Uo(i,j1+2)+8*Uo(i,j1+1)-8*Uo(i,Nx)+Uo(i,Nx-1)
        elseif j1 == 2
            a1x(1,j1)=2+a*86400*(-U(i,j1+2)+8*U(i,j1+1)-8*U(i,j1-1)+U(i,Nx)-Uo(i,j1+2)+8*Uo(i,j1+1)-8*Uo(i,j1-1)+Uo(i,Nx))+30*(a/dx)*K(i,j1);%-Uo(i,j1+2)+8*Uo(i,j1+1)-8*Uo(i,j1-1)+Uo(i,Nx)
        elseif j1 == 49
            a1x(1,j1)=2+a*86400*(-U(i,1)+8*U(i,j1+1)-8*U(i,j1-1)+U(i,j1-2)-Uo(i,1)+8*Uo(i,j1+1)-8*Uo(i,j1-1)+Uo(i,j1-2))+30*(a/dx)*K(i,j1);%-Uo(i,1)+8*Uo(i,j1+1)-8*Uo(i,j1-1)+Uo(i,j1-2)
        elseif j1 == 50
            a1x(1,j1)=2+a*86400*(-U(i,2)+8*U(i,1)-8*U(i,j1-1)+U(i,j1-2)-Uo(i,2)+8*Uo(i,1)-8*Uo(i,j1-1)+Uo(i,j1-2))+30*(a/dx)*K(i,j1);%-Uo(i,2)+8*Uo(i,1)-8*Uo(i,j1-1)+Uo(i,j1-2)
        else
            a1x(1,j1)=2+a*86400*(-U(i,j1+2)+8*U(i,j1+1)-8*U(i,j1-1)+U(i,j1-2)-Uo(i,j1+2)+8*Uo(i,j1+1)-8*Uo(i,j1-1)+Uo(i,j1-2))+30*(a/dx)*K(i,j1);%-Uo(i,j1+2)+8*Uo(i,j1+1)-8*Uo(i,j1-1)+Uo(i,j1-2)
        end
        
        Nowx(j1,1)=mat(j1,i);
    end
    for j2=1:Nx-1
        a2x(1,j2)= -1*8*U(i,j2)*86400*a +(-1*16*K(i,j2)*86400*(a/dx));
    end
    for j3=2:Nx
        a3x(1,j3-1)= 8*U(i,j3)*86400*a +(-1*16*K(i,j3)*86400*(a/dx));
    end
    for j4=1:Nx-2
        a4x(1,j4)= U(i,j4)*86400*a + K(i,j4)*86400*(a/dx);
    end
    for j5=3:Nx
        a5x(1,j5-2)= -1*U(i,j5)*86400*a + K(i,j5)*86400*(a/dx);
    end
       
        Mlx = diag(a1x,0)+diag(a2x,1)+diag(a3x,-1)+diag(a4x,2)+diag(a5x,-2);
        Mlx(1,Nx-1)=-1*U(i,1)*86400*a + K(i,1)*86400*(a/dx);
        Mlx(1,Nx)=8*U(i,1)*86400*a + (-1*16*K(i,1)*86400*(a/dx));
        Mlx(2,Nx)=-1*U(i,2)*86400*a + K(i,2)*86400*(a/dx);
        Mlx(Nx-1,1)=U(i,1)*86400*a + K(i,1)*86400*(a/dx);
        Mlx(Nx,1)=-1*8*U(i,1)*86400*a + (-1*16*K(i,1)*86400*(a/dx));
        Mlx(Nx,2)=U(i,2)*86400*a + K(i,2)*86400*(a/dx);
        
        
    for j1=1:Nx
        if j1 == 1
            a1xx(j1,1)=2+a*86400*(-U(j1+2,i)+8*U(j1+1,i)-8*U(Nx,i)+U(Nx-1,i)-Uo(j1+2,i)+8*Uo(j1+1,i)-8*Uo(Nx,i)+Uo(Nx-1,i));%-Uo(j1+2,i)+8*Uo(j1+1,i)-8*Uo(Nx,i)+Uo(Nx-1,i)
        elseif j1 == 2
